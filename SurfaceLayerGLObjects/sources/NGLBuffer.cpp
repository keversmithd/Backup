...
int   NGLBuffer::BindBufferBase(GLenum target, GLuint first, GLsizei count)
{
	if (deleted)
	{
		printf("This buffer is deleted and has be to generated again.\n");
	...
	glBindBuffersBase(target, first, count, &m_buffer);
	GetError
		if (Code) { return 0; }
...
int   NGLBuffer::BufferSubData(GLuint buffer, GLintptr offset, GLsizei size, const void* data)
{
...
int   NGLBuffer::UnmapBuffer()
{
	...
	if (mapped)
	{
		GLboolean mapped = glUnmapNamedBuffer(m_buffer);
		GetError
			if (Code) { return 0; }
		mapped = false;
		return 1;
